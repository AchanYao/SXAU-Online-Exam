<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="课程编号" width="150px">
        <template slot-scope="{row}">
      ...
      <el-table-column align="center" label="Name" width="300px">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="课程组" width="150px">
        <template slot-scope="{row}">
          <el-button size="mini" @click="toTeacherGroup(row.id)">查看</el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="课程名称（英）" prop="name">
          <el-input v-model="temp.name" />
        </el-form-item>
        <el-form-item label="课程名称（中）" prop="nameZh">
          <el-input v-model="temp.nameZh" />
        </el-form-item>
      </el-form>
...
import { addCourse, deleteCourseById, pageCourses, updateCourse } from '../../api/courses'
import { getGroupByCourse } from '../../api/teacher-group'
...
export default {
  ...
  methods: {
    getList() {
      ...
      pageCourses(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addCourse(this.temp).then((result) => {
    ...
    toTeacherGroup(courseId) {
      getGroupByCourse(courseId).then(result => {
        if (result.data === null) {
          this.$message.info('该课程未设置课程组')
        } else {
          this.$router.push('/teacher-groups/details/' + result.data.id)
        }
      })
    }
  }
